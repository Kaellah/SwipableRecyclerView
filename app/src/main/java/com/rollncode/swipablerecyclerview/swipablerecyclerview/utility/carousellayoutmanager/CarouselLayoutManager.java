package com.rollncode.swipablerecyclerview.swipablerecyclerview.utility.carousellayoutmanager;
...
public class CarouselLayoutManager extends RecyclerView.LayoutManager {
    ...
    public boolean canScrollHorizontally() {
        return false;//0 != getChildCount() && HORIZONTAL == mOrientation;
    }
...
//        final int centerViewTop = (height - mDecoratedChildHeight) / 2;
    ...
    private void fillChildItem(final int start, final int top, final int end, final int bottom, @NonNull final LayoutOrder layoutOrder, @NonNull final RecyclerView.Recycler recycler, final int i, final boolean childMeasuringNeeded) {
        ...
        if (null == transformation) {
            ...
            view.layout(Math.round(start + transformation.mTranslationX), Math.round(top + transformation.mTranslationY), Math.round(end + transformation.mTranslationX), Math.round(bottom + transformation.mTranslationY));
